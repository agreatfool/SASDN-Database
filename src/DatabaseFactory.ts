...
export class DatabaseFactory {
   ...
   * @param entityPath 
   */
  async checkShardTable(entityPath: string): Promise<string[]> {
    let entitiesPath: Set<string> = new Set();
    try {
  ...
  async createClusterConnections(options: Array<ClusterOptions>): Promise<Map<string, Connection[]>> {
    ...
    try {
      for (let cluster of options) {
        for(let conn of cluster.cluster) {
          let finalPath:Set<string> = new Set();
          let entities:string[] = conn.entities as string[];
        }
